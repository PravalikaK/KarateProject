Feature:  karate db test suite
  ...
  Scenario: insert stmt test case
    ...
    * def res = db.insertStatement("Insert into tools values(188,'sample2','xyz')")
    * print res
    * match res == 'Success'

  Scenario: update stmt test case

    * def rowsAffected = db.updateStatement("update tools set name='Warrior' where id=188")
    * print rowsAffected
    * assert rowsAffected >= 1

  Scenario: delete stmt test case

    * def rowsAffected = db.deleteStatement("delete from tools where id=188")
    * print rowsAffected
    * assert rowsAffected >= 1
